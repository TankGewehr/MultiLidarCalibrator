                                                  ...
                                                  const sensor_msgs::PointCloud2::ConstPtr &in_child_cloud_msg)
{
	...
	saveExtrinsic(extrinsic_json_path,extrinsic);
	// timer end
...
void ROSMultiLidarCalibratorApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
	...
	in_private_handle.param<std::string>("extrinsic_json_path", extrinsic_json_path, "lidar.json");
	ROS_INFO("[%s] extrinsic_json_path: %s",__APP_NAME__, extrinsic_json_path.c_str());
